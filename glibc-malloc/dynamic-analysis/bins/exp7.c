...
/* OBJECTIVE: A largebin is basically a collection of fixed size classes. */
...
/* GIVEN: 
  ...
  3. The number of fixed size classes the bins in this largebin category can contain are 4 (BIN_WIDTH/SMALLBIN_WIDTH, i.e. 64/16). They are 1024 (1008+(16*1)), 1040 (1008+(16*2)), 1056 (1008+(16*3)), 1072 (1008+(16*4)).
...
Take the head address and dereference it. Take the address in fd and dereference it. Repeat until we reach the header of this bin.

Because all the freed chunks are found in a single bin, that proves that each largebin is basically a collection of fixed size classes.
