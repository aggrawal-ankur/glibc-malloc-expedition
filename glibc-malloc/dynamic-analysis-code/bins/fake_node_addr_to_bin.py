...
def main_logic():
  ...
  for BIN_NUM in range(2, 63+1):
    print(f"| Bin #{BIN_NUM} | {SMALL_SIZES[BIN_NUM-2]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16

  for BIN_NUM in range(64, 64+32):
    print(f"| Bin #{BIN_NUM} | {LARGEBIN_CAT1[BIN_NUM-64]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16

  for BIN_NUM in range(96, 96+16):
    print(f"| Bin #{BIN_NUM} | {LARGEBIN_CAT2[BIN_NUM-96]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16

  for BIN_NUM in range(112, 112+8):
    print(f"| Bin #{BIN_NUM} | {LARGEBIN_CAT3[BIN_NUM-112]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16

  for BIN_NUM in range(120, 120+4):
    print(f"| Bin #{BIN_NUM} | {LARGEBIN_CAT4[BIN_NUM-120]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16

  for BIN_NUM in range(124, 124+2):
    print(f"| Bin #{BIN_NUM} | {LARGEBIN_CAT5[BIN_NUM-124]} | (bins[{BIN_HDRS}], bins[{BIN_HDRS+1}]) | <main_arena+{ARENA_OFFSET}> |")
    BIN_HDRS += 2
    ARENA_OFFSET += 16
